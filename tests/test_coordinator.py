...
@pytest.mark.asyncio
async def test_existing_config_entry_connects_through_packaged_client(monkeypatch):
    ...
    class FakeXSense:
        def __init__(self, session, language=None):
            calls.append(("init_client", session, language))
    ...
    monkeypatch.setattr(coordinator_module, "AsyncXSense", FakeXSense)
    monkeypatch.setattr(
        coordinator_module,
        "async_get_clientsession",
        lambda hass: "client-session",
    )
    ...
    coordinator.xsense = None
    coordinator._initialized = True
    coordinator._camera_initialized = True
    coordinator._last_camera_update_attempt = object()
    coordinator._camera_station_cache = {"old": object()}
    coordinator.mqtt_servers = {}
    coordinator.logger = logging.getLogger(DOMAIN)

    await XSenseDataUpdateCoordinator._connect(coordinator)

    assert calls == [
        ("init_client", "client-session", "de"),
        ("init",),
        ("login", "user@example.com", "secret"),
    ]
    assert isinstance(coordinator.xsense, FakeXSense)
    ...
    assert coordinator._camera_station_cache == {}
    assert entry.options == {CONF_RECORDING_MEDIA_SYNC_ENABLED: True}
