...
with warnings.catch_warnings():
    ...
    try:
        ...
        AiortcRTCBundlePolicy = None
        AiortcRTCConfiguration = None
        AiortcRTCIceServer = None
...
def _payload_debug(payload: Any) -> str:
    if isinstance(payload, dict):
        return f"dict_keys={sorted(str(key) for key in payload)}"
...
def make_start_live_data_channel_command_payload(
    ...
    *,
    ...
    """Return the APK startLive data-channel command as JSON."""
    payload = make_data_channel_command(
        "startLive",
...
def make_sd_video_list_command_payload(
    ...
    *,
    ...
    """Return the APK getSdVideoList data-channel command as JSON."""
    payload = make_data_channel_command(
        "getSdVideoList",
        request_id=request_id,
        parameters={"startTime": int(start_time), "stopTime": int(stop_time)},
    )
    if timestamp is not None:
        payload["timeStamp"] = int(timestamp)
    return json.dumps(payload, separators=(",", ":"))
    ...
    def __init__(
        ...
        *,
        ...
        if RTCPeerConnection is None or RTCSessionDescription is None:
            raise RuntimeError("Home Assistant WebRTC media stack is not available")
