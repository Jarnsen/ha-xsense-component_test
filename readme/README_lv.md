...
Atbalstītās kameras pēc noklusējuma izmanto stabilo X-Sense stream source ceļu Home Assistant tiešraidei ar video un audio, ja kamera/konts to nodrošina. Eksperimentālo X-Sense WebRTC tiltu var ieslēgt integrācijas opcijās testēšanai, un tas ieslēdz debug žurnālus. Kameras izveido `Motion` un `AI Detection` notikumu entītijas.
...
Atbalstītās kameras var nodrošināt kameras entītiju, sīktēlus, tiešraides straumi, savienojuma statusu un iestatījumus, kas atbilst X-Sense lietotnei. Kameras tiešraide pēc noklusējuma izmanto stabilo X-Sense stream source ceļu; eksperimentālais X-Sense WebRTC tilts tiek izmantots tikai tad, ja tas izvēlēts integrācijas opcijās testēšanai.
...
- Kameras tiešraide pēc noklusējuma izmanto stabilo X-Sense stream source ceļu; eksperimentālais X-Sense WebRTC tilts ir paredzēts tikai testēšanai integrācijas opcijās.
