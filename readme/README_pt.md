...
O movimento da câmara está disponível como sensor binário Detetado/Livre e também como um evento Motion único para cada nova deteção. AI Detection continua a ser um evento único. Para automações manuais, use o gatilho `event.received` do Home Assistant com a entidade de câmera `Motion` ou `AI Detection`; `event_type` só é necessário para limitar AI Detection com assinatura a tipos como `person`, `pet`, `vehicle`, `package`, `other` ou `ai_detection`.
