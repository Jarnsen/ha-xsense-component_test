...
Se você ainda usa uma versão antiga `v1.2.6.x`, atualize para `v1.3.14` ou mais recente antes de atualizar o Home Assistant Core para 2026.7 ou mais recente. As versões antigas `v1.2.6.x` exigiam `aiortc`, que não é compatível com o runtime Python 3.14 do Home Assistant. As versões atuais `v1.3.x` não exigem mais `aiortc`.
...
Quando um evento Motion inclui metadados de reprodução da X-Sense, a integração tenta colocar o clipe em cache imediatamente. Quando o arquivo está pronto, a notificação usa `recording_media_url` para reprodução de vídeo; caso contrário, `recording_url` abre o visualizador X-Sense Recordings no Home Assistant. Execuções manuais da automação usam os dados de gravação mais recentes da entidade de evento selecionada, portanto a notificação de gravação só é enviada depois que essa entidade recebe um link de gravação real. Recording media sync pode preparar clipes recentes em segundo plano.
