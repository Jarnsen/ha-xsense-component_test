...
Jeśli nadal używasz starej wersji `v1.2.6.x`, zaktualizuj ją do `v1.3.14` lub nowszej przed aktualizacją Home Assistant Core do wersji 2026.7 lub nowszej. Stare wersje `v1.2.6.x` wymagały `aiortc`, które nie jest zgodne ze środowiskiem Python 3.14 w Home Assistant. Aktualne wersje `v1.3.x` nie wymagają już `aiortc`.
...
Gdy zdarzenie Motion zawiera metadane odtwarzania X-Sense, integracja od razu próbuje zapisać klip w cache. Gdy plik jest gotowy, powiadomienie używa `recording_media_url` do odtwarzania wideo; w przeciwnym razie `recording_url` otwiera widok X-Sense Recordings w Home Assistant. Ręczne uruchomienia automatyzacji używają najnowszych danych nagrania z wybranej encji zdarzenia, więc powiadomienie z nagraniem jest wysyłane dopiero po otrzymaniu przez tę encję prawdziwego linku do nagrania. Recording media sync może przygotowywać najnowsze klipy w tle.
