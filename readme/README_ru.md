...
Интеграция создает сущности только для полей из облака X-Sense, MQTT shadow или API камеры, подтвержденных приложением. Это могут быть бинарные сенсоры, диагностические сенсоры, переключатели, списки выбора, числовые настройки и кнопки для поддерживаемых функций, например тест, отключение звука, пожарная тренировка и пробуждение камеры.

Если поле отсутствует или приложение X-Sense помечает функцию как неподдерживаемую для этого устройства/аккаунта, сущность не создается. Привязка, удаление, общий доступ, аккаунт, платежи, прошивка, форматирование SD-карты и другие административные действия остаются в приложении X-Sense.
