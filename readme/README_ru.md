...
Если вы всё ещё используете старую сборку `v1.2.6.x`, обновитесь до `v1.3.14` или новее перед обновлением Home Assistant Core до 2026.7 или новее. Старые сборки `v1.2.6.x` требовали `aiortc`, который несовместим со средой Python 3.14 в Home Assistant. Текущие версии `v1.3.x` больше не требуют `aiortc`.
...
Когда событие Motion содержит данные воспроизведения X-Sense, интеграция может сначала сохранить клип в кэш, а затем отправить мобильное уведомление, которое открывает соответствующий клип в X-Sense Recordings. Отключите ссылки на записи в blueprint, если нужно простое уведомление о движении без ожидания видео. Синхронизация медиа записей может заранее готовить новые клипы в фоне, а старые импортированные blueprint камер X-Sense обновляются автоматически.
