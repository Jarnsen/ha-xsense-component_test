...
Cuando un evento Motion incluye metadatos de reproducción de X-Sense, la integración intenta guardar el clip en caché de inmediato. Cuando el archivo está listo, la notificación usa `recording_media_url` para reproducir el video; si no, `recording_url` abre el visor X-Sense Recordings en Home Assistant. Las ejecuciones manuales de la automatización usan los datos de grabación más recientes de la entidad de evento seleccionada, por lo que solo envían una notificación de grabación después de que esa entidad haya recibido un enlace de grabación real. Recording media sync puede preparar clips recientes en segundo plano.
