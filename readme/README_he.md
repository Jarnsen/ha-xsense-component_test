...
כאשר אירוע Motion כולל מטא-נתוני playback של X-Sense, האינטגרציה מנסה לשמור מיד את הקליפ במטמון. כשהקובץ מוכן, ההתראה משתמשת ב-`recording_media_url` להפעלת הווידאו; אחרת `recording_url` פותח את מציג X-Sense Recordings בתוך Home Assistant. Recording media sync יכול להכין קליפים חדשים ברקע.
