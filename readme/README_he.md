...
## תאימות ועדכוני HACS
אם אתה עדיין משתמש בגרסת `v1.2.6.x` ישנה, עדכן ל-`v1.3.14` או חדשה יותר לפני שדרוג Home Assistant Core ל-2026.7 או חדש יותר. הגרסאות הישנות דרשו `aiortc`, שאינו תואם לסביבת Python 3.14 של Home Assistant. גרסאות `v1.3.x` הנוכחיות כבר אינן דורשות `aiortc`.
...
כאשר אירוע Motion כולל מטא-נתוני playback של X-Sense, האינטגרציה מנסה לשמור מיד את הקליפ במטמון. כשהקובץ מוכן, ההתראה משתמשת ב-`recording_media_url` להפעלת הווידאו; אחרת `recording_url` פותח את מציג X-Sense Recordings בתוך Home Assistant. הרצה ידנית של האוטומציה משתמשת בנתוני ההקלטה האחרונים של ישות האירוע שנבחרה, לכן התראת הקלטה נשלחת רק אחרי שהישות קיבלה קישור הקלטה אמיתי. Recording media sync יכול להכין קליפים חדשים ברקע.
