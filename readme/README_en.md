...
The easiest UI path is the included blueprint. Use the button below to import it, select the camera `Motion` event entity, or `AI Detection` when it is available for a subscribed camera, then keep or replace the default notification action. When a Motion event includes playback metadata, the integration immediately tries to download the new clip, then fires the event with `recording_url` and, when the cached file is ready, `recording_media_url`. The default mobile notification attaches cached media as Companion app video when available and otherwise opens the X-Sense Recordings sidebar playback link. Manual automation runs use the selected event entity's latest recording data, so they can only send a recording notification after that entity has received a real recording link. Recording media sync can also keep recent clips ready in the background. If a mobile notification action fails because a phone is not connected to local push notifications, edit the blueprint automation action and choose a working notification target.
