...
Camera SD-card recordings appear in the X-Sense Recordings sidebar and under Media Browser > X-Sense Recordings after the integration has refreshed the APK recording history. The list is grouped by camera and date and uses a local cache for faster browsing. Clips are cached under `/media/xsense_recordings` on first play by default; the cache folder can be changed in the integration options, but move the existing `videos` and `thumbs` contents to the new folder before changing it if you want old cached recordings to remain available. The options also set cache retention (7 days by default) and a maximum cache size (2 GB by default). Automatic cleanup applies whichever limit is reached first and removes least-recently-viewed clips first. The Recordings panel can delete one cached clip, one camera's cache, or all cached recordings. Cleanup only removes Home Assistant's local copies; recordings on X-Sense SD cards or cloud storage are not changed. Newest clips are synchronized first, older clips catch up in the background sweep, and the motion or AI event playback metadata is used as the priority path for fresh notification clips.
