...
- Supported camera SD-card recording history is available from Home Assistant's Media Browser when X-Sense reports APK playback metadata for the clips.
...
The easiest UI path is the included blueprint. Use the button below to import it, select the camera `Motion` event entity, or `AI Detection` when it is available for a subscribed camera, then keep or replace the default notification action. The default mobile notification can include recording and snapshot links when X-Sense provides them for a Motion event, and tapping the notification opens the recording when one is available. For best recording playback results, enable **Recording media sync** in the integration options. The integration keeps a broad background catch-up sync for older clips, checks for recent clips every couple minutes, and prepares motion or AI event recordings before firing the event notification when playback metadata is available. `recording_url` opens a Home Assistant playback page for mobile notification taps; `recording_media_url` is also available when cached Home Assistant media is ready. If a mobile notification action fails because a phone is not connected to local push notifications, edit the blueprint automation action and choose a working notification target.
