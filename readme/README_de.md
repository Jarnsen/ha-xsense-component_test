...
Wenn ein Motion-Event X-Sense-Wiedergabedaten enthält, versucht die Integration den Clip sofort zu cachen. Sobald die Datei bereit ist, nutzt die Benachrichtigung `recording_media_url` für die Videowiedergabe; andernfalls öffnet `recording_url` den X-Sense-Recordings-Viewer in Home Assistant. Manuelle Automationsläufe verwenden die neuesten Aufzeichnungsdaten der ausgewählten Ereignis-Entität, daher wird eine Aufzeichnungsbenachrichtigung erst gesendet, nachdem diese Entität einen echten Aufzeichnungslink erhalten hat. Recording media sync kann aktuelle Clips im Hintergrund vorbereiten.
