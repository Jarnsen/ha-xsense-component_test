...
- Supported cameras expose Motion and AI Detection updates through Home Assistant event entities, with a simple automation blueprint available in the language documentation. Motion events preserve APK record playback metadata and expose `recording_url` when X-Sense reports a direct playback URL; the blueprint can include recording and snapshot links in the notification, and mobile notifications can open the recording when tapped. AI Detection remains separate and depends on the camera/account AI service state.
