...
- Supported cameras expose Motion and AI Detection updates through Home Assistant event entities, with a simple automation blueprint available in the language documentation. Motion events preserve APK record playback metadata when X-Sense reports it, while AI Detection remains separate and depends on the camera/account AI service state.
