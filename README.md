...
- Supported cameras expose Motion and AI Detection updates through Home Assistant event entities, with a simple automation blueprint available in the language documentation. Motion events preserve APK record playback metadata, immediately try to cache the new clip, and expose `recording_url` plus `recording_media_url` when cached Home Assistant media is ready. The blueprint attaches cached media as Companion app video when available and otherwise opens the X-Sense Recordings sidebar playback link. Manual blueprint runs use the selected event entity's latest recording data, so they only send a recording notification after that entity has received a real recording link. Recording media sync can also keep recent clips ready in the background. AI Detection remains separate and depends on the camera/account AI service state.
- Supported camera SD-card recording history is exposed through the X-Sense Recordings sidebar viewer and Home Assistant's Media Browser when X-Sense reports APK playback metadata for the clips. Direct X-Sense video URLs are cached as media, and SD-only recordings are captured through the bundled no-transcode Pion adapter when playback is requested or when recording media sync prepares the clip in the background. Recording media sync processes newest clips first while older clips catch up in the normal background sweep. The recording cache folder is configurable under integration options; move existing cached `videos` and `thumbs` contents before changing it if you want old cached recordings to remain available.
