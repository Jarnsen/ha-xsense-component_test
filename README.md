...
- Supported cameras expose Motion and AI Detection updates through Home Assistant event entities, with a simple automation blueprint available in the language documentation. Motion events preserve APK record playback metadata and expose `recording_url`; when event caching succeeds this points to cached Home Assistant media, otherwise it stays on the Home Assistant recording route fallback. The blueprint can include recording and snapshot links in the notification, and mobile notifications can open the recording when tapped. For best notification playback results, enable Recording media sync. The integration checks recent clips every couple minutes and prepares motion or AI event recordings before firing the notification when playback metadata is available. AI Detection remains separate and depends on the camera/account AI service state.
- Supported camera SD-card recording history is exposed through Home Assistant's Media Browser when X-Sense reports APK playback metadata for the clips. Direct X-Sense video URLs are cached as media, and SD-only recordings are captured through the bundled no-transcode Pion adapter when playback is requested or when recording media sync prepares the clip in the background. Recording media sync processes newest clips first while older clips catch up in the normal background sweep. The recording cache folder is configurable under integration options; move existing cached `videos` and `thumbs` contents before changing it if you want old cached recordings to remain available.
