...
- Supported cameras expose Motion and AI Detection updates through Home Assistant event entities, with a simple automation blueprint available in the language documentation. Motion events preserve APK record playback metadata and expose `recording_url` as a Home Assistant playback page that mobile notifications can open when tapped. When event caching succeeds, `recording_media_url` also points to the cached Home Assistant media file. For best notification playback results, enable Recording media sync. The integration checks recent clips every couple minutes and prepares motion or AI event recordings before firing the notification when playback metadata is available. AI Detection remains separate and depends on the camera/account AI service state.
