...
- Supported cameras expose Motion and AI Detection updates through Home Assistant event entities, with a simple automation blueprint available in the language documentation. Motion events preserve APK record playback metadata, immediately cache the new clip, and expose `recording_url` plus `recording_media_url` after the cached Home Assistant media is ready. The blueprint sends the mobile notification only after `recording_media_url` is available, attaches it as Companion app video media, and uses `recording_url` to open the X-Sense Recordings sidebar viewer. Recording media sync can also keep recent clips ready in the background. AI Detection remains separate and depends on the camera/account AI service state.
